...
public class LoadSquaresContinue implements Runnable{
    ...
    // 迭代用构造器
    LoadSquaresContinue(int id , CellMap Map){     // id表示线程的编号，用于确定Start (Start的算法： if id == 1 -> Start = height-1 else Start = (id-1)*(Length-2)-1)
        this.Map = Map;
        this.id = id;
        ...
        synchronized (this) {
            ...
            if (Start != height - 1 && (Start + Length) < (height - 2)) {
                ...
                for (int i = 0; i < height; i++) {      // 将对应位置的像素信息读入map数组中
                    for (int j = 0; j < Width; j++) {
                        if (i >= Start && i < Start + Length)
                            map[i - Start][j] = Map.getValue(i, j);
                    }
                }
                ...
                for (int i = 0; i < height; i++) {      // 将对应位置的像素信息读入map数组中
                    for (int j = 0; j < Width; j++) {
                        if (i < Length - 1)
                            map[i + 1][j] = Map.getValue(i, j);
                        else if (i == height - 1)
                            map[0][j] =  Map.getValue(i, j);
                    }
                }
            } else {
                // 读取下边缘
                for (int i = 0; i < height; i++) {      // 将对应位置的像素信息读入map数组中
                    for (int j = 0; j < Width; j++) {
                        if (i >= Start) {
                            ...
                            map[Length - 1][j] =  Map.getValue(i, j);
                        }
                    }
                }
            }
